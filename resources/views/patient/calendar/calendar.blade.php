        ...
        <tbody>
            @foreach ($calendars as $calendar)
                <tr>
                    ...
                    <td>{{ $calendar->time }}</td>
                    <td>
                        <a href="{{ route('patient.updateCalendar', $calendar->id) }}" class="btn btn-light"title="Update"><i class="fa-solid fa-pen"></i></a>
                        <form method="post" action="{{ route('patient.deleteCalendar', $calendar->id) }}" style="display: inline;">
                            @csrf
                            @method('DELETE')
